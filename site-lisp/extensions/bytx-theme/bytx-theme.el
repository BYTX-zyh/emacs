...
(defgroup bytx-themes nil
  "Options for bytx-themes."
  :group 'faces)

(defcustom bytx-themes-padded-modeline nil
  "Default value for padded-modeline setting for themes that support it."
  :group 'bytx-themes
  :type '(choice integer boolean))

;;
(defcustom bytx-themes-enable-bold t
  "如果为空,将会在所有 face 中禁用 字体加粗(bold)."
  :group 'bytx-themes
  :type 'boolean)

(defcustom bytx-themes-enable-italic t
...
(defun bytx-themes--colors-p (item)
  "检查 item 参数是否包含颜色"

  ;; 声明为纯函数,结果只与参数有关且不会产生副作用
  (declare (pure t) (side-effect-free t))

  (when item
    (cond ((listp item) ;; 如果参数 item为list
           (let ((car (car item))) ;; car
             ;; 如果 car 为 bytx-color 的元素,则返回nil
             (cond ((memq car '(quote bytx-color)) nil)
                   ;; backquote 设置 bytx-quote-p 为 t ,对剩余部分递归调用
                   ((memq car '(backquote \`))
                    (let ((bytx--quoted-p t))
                      (bytx-themes--colors-p (cdr item))))
                   ;; 逗号表达式
                   ((eq car '\,)
                    (let (bytx--quoted-p)
                      (bytx-themes--colors-p (cdr item))))
                   ;; car / cdr 满足
                   ((or (bytx-themes--colors-p car)
                        (bytx-themes--colors-p (cdr-safe item)))))))

          ;; 是 symbol 不是 keyword ,lazy-cat--quoted-p 为false, 开头不是 - ,返回对应的值
          ((and (symbolp item)
                (not (keywordp item))
                (not bytx--quoted-p)
                (not (equal (substring (symbol-name item) 0 1) "-"))
                (assq item bytx-themes--colors))))))

(defun bytx-themes--apply-faces (new-faces &optional default-faces)
  (declare (pure t) (side-effect-free t)) ;; 声明为不产生副作用的纯函数
  (let ((default-faces (or default-faces bytx-themes-base-faces)) ;; 如果未传递 default-faces 则使用 bytx-themes-base-faces 作为 default-faces
        (faces (make-hash-table :test #'eq :size (+ (length default-faces) (length new-faces)))) ;; 创造 hash表 faces ,其大小为 default-faces 与 new-faces 的总和,其判定相同的条件为键值均相等
        (directives (make-hash-table :test #'eq))) ;; hash 存储特殊face

    (dolist (spec (append (mapcar #'copy-sequence default-faces) new-faces)) ;; 将 default-faces 浅复制后与 new-faces 合并为 spec 作为 dolist的内容
      (if (listp (car spec)) ;; 如果spec的首个元素为lisp
          ;; 将 (car spec) 分给 (face action &optional arg) 并执行后续代码
          ;; 前两项绑定为face与action,其余项为arg
          ;; 如果 new-faces 中不存在face , 将其整合后添加到 directives
    ...
    (let (results)
      (maphash (lambda (face plist)
                 (when (keywordp (car plist))
                   ...
                   (dolist (prop (append (unless bytx-themes-enable-bold   '(:weight normal :bold nil))
                                         (unless bytx-themes-enable-italic '(:slant normal :italic nil))))
...
(defun bytx-themes--colorize (item type)
  (declare (pure t) (side-effect-free t))
  (when item
    (let ((bytx--quoted-p bytx--quoted-p))
      (cond ((listp item)
             (cond ((memq (car item) '(quote bytx-color))
                    item)
                   ((eq (car item) 'bytx-ref)
                    (bytx-themes--colorize
                     (apply #'bytx-ref (cdr item)) type))
                   ((let* ((item (append item nil))
                      ...
                      (cons car
                            (cl-loop
                             for i in (cdr item)
                             collect (bytx-themes--colorize i type)))))))
            ...
            ((and (symbolp item)
                  ...
                  (not (equal (substring (symbol-name item) 0 1) "-"))
                  (assq item bytx-themes--colors))
             `(bytx-color ',item ',type))

            (item)))))

(defun bytx-themes--build-face (face)
  (declare (pure t) (side-effect-free t))
  `(list
    ',(car face)
    ,(let ((face-body (cdr face)))
       (cond ((keywordp (car face-body))
              (let ((real-attrs face-body)
                    defs)
                (if (bytx-themes--colors-p real-attrs)
                    (dolist (cl bytx--min-colors `(list ,@(nreverse defs)))
                      (push `(list '((class color) (min-colors ,cl))
                                   (list ,@(bytx-themes--colorize real-attrs cl)))
                            defs))
                  ...
                  (cond ((eq (car attrs) '&all)
                        ...
                        ((memq (car attrs) '(&dark &light))
                         (let ((bg (if (eq (car attrs) '&dark) 'dark 'light))
                           ...
                           (cond ((bytx-themes--colors-p real-attrs)
                                  (dolist (cl bytx--min-colors)
...
;;;###autoload
(defun bytx-blend (color1 color2 alpha)
  "Blend two colors (hexidecimal strings) together by a coefficient ALPHA (a
float between 0 and 1)"
  (when (and color1 color2)
    (cond ((and color1 color2 (symbolp color1) (symbolp color2))
           (bytx-blend (bytx-color color1) (bytx-color color2) alpha))

          ((or (listp color1) (listp color2))
           (cl-loop for x in color1
                    when (if (listp color2) (pop color2) color2)
                    collect (bytx-blend x it alpha)))

          ((and (string-prefix-p "#" color1) (string-prefix-p "#" color2))
           (apply (lambda (r g b) (format "#%02x%02x%02x" (* r 255) (* g 255) (* b 255)))
                  (cl-loop for it    in (bytx-name-to-rgb color1)
                           for other in (bytx-name-to-rgb color2)
                           collect (+ (* alpha it) (* other (- 1 alpha))))))

          (color1))))

;;;###autoload
(defun bytx-darken (color alpha)
  "Darken a COLOR (a hexidecimal string) by a coefficient ALPHA (a float between
0 and 1)."
  (cond ((and color (symbolp color))
         (bytx-darken (bytx-color color) alpha))

        ((listp color)
         (cl-loop for c in color collect (bytx-darken c alpha)))

        ((bytx-blend color "#000000" (- 1 alpha)))))

;;;###autoload
(defun bytx-lighten (color alpha)
  "Brighten a COLOR (a hexidecimal string) by a coefficient ALPHA (a float
between 0 and 1)."
  (cond ((and color (symbolp color))
         (bytx-lighten (bytx-color color) alpha))

        ((listp color)
         (cl-loop for c in color collect (bytx-lighten c alpha)))

        ((bytx-blend color "#FFFFFF" (- 1 alpha)))))

;;;###autoload
(defun bytx-color (name &optional type)
  "Retrieve a specific color named NAME (a symbol) from the current theme."
  (let ((colors (if (listp name)
                    name
                  (cdr-safe (assq name bytx-themes--colors)))))
    (and colors
         (cond ((listp colors)
...
;;;###autoload
(defun bytx-ref (face prop &optional class)
  "TODO"
  (let ((spec (or (cdr (assq face bytx-themes--faces))
...
(defun bytx-themes-prepare-facelist (custom-faces)
  "Return an alist of face definitions for `custom-theme-set-faces'.

Faces in EXTRA-FACES override the default faces."
  (declare (pure t) (side-effect-free t)) ;; 声明为纯函数,不产生副作用
  (setq bytx-themes--faces (bytx-themes--apply-faces custom-faces))
  ;; 对 bytx-themes--faces 内的每个值执行 bytx-themes--build-face
  (mapcar #'bytx-themes--build-face bytx-themes--faces))

(defun bytx-themes-prepare-varlist (vars)
  "Return an alist of variable definitions for `custom-theme-set-variables'.

Variables in EXTRA-VARS override the default ones."
  (declare (pure t) (side-effect-free t))
  (cl-loop for (var val) in (append bytx-themes-base-vars vars)
           collect `(list ',var ,val)))

;;;###autoload
(defun bytx-themes-set-faces (theme &rest faces)
  "Customize THEME (a symbol) with FACES.

If THEME is nil, it applies to all themes you load. FACES is a list of bytx
theme face specs. These is a simplified spec. For example:

  (bytx-themes-set-faces 'user
  ...
  (apply #'custom-theme-set-faces
         (or theme 'user)
         (eval
          `(let* ((bold   ,bytx-themes-enable-bold)
                  (italic ,bytx-themes-enable-italic)
                  ,@(cl-loop for (var . val) in bytx-themes--colors
                             collect `(,var ',val)))
  ...
  (let ((bytx-themes--colors defs)) ;; defs的值为颜色对照表,将其赋给bytx-themes--colors
    ...
    `(let* ((bold   bytx-themes-enable-bold) ;; 是否允许加粗
       ...
       (custom-theme-set-faces
        ',name ,@(bytx-themes-prepare-facelist extra-faces))
...
(defvar bytx-themes-base-faces
  '(
    ...
    (custom-saved             :foreground green :background (bytx-blend green bg 0.2) :bold bold)
    (custom-comment           :foreground fg :background region)
    (custom-comment-tag       :foreground grey)
    (custom-modified          :foreground blue :background (bytx-blend blue bg 0.2))
    ...
    (custom-invalid           :foreground red :background (bytx-blend red bg 0.2))
    (custom-variable-obsolete :foreground grey :background bg)
    (custom-state             :foreground green :background (bytx-blend green bg 0.2))
    ...
    ;; ediff
    (ediff-fine-diff-A    :background (bytx-blend selection bg 0.7) :weight 'bold :extend t)
    (ediff-fine-diff-B    :inherit 'ediff-fine-diff-A)
    (ediff-fine-diff-C    :inherit 'ediff-fine-diff-A)
    (ediff-current-diff-A :background (bytx-blend selection bg 0.3) :extend t)
    ...
    (message-header-cc         :inherit 'message-header-to :foreground (bytx-darken highlight 0.15))
    ...
    ;; vterm
    (vterm               :foreground fg)
    (vterm-color-black   :background (bytx-lighten base0 0.25)   :foreground base0)
    (vterm-color-red     :background (bytx-lighten red 0.25)     :foreground red)
    (vterm-color-green   :background (bytx-lighten green 0.25)   :foreground green)
    (vterm-color-yellow  :background (bytx-lighten yellow 0.25)  :foreground yellow)
    (vterm-color-blue    :background (bytx-lighten blue 0.25)    :foreground blue)
    (vterm-color-magenta :background (bytx-lighten magenta 0.25) :foreground magenta)
    (vterm-color-cyan    :background (bytx-lighten cyan 0.25)    :foreground cyan)
    (vterm-color-white   :background (bytx-lighten base8 0.25)   :foreground base8)
    ...
    (all-the-icons-lred     :foreground (bytx-lighten red 0.3))
    (all-the-icons-dred     :foreground (bytx-darken red 0.3))
    (all-the-icons-green    :foreground green)
    (all-the-icons-lgreen   :foreground (bytx-lighten green 0.3))
    (all-the-icons-dgreen   :foreground (bytx-darken green 0.3))
    (all-the-icons-yellow   :foreground yellow)
    (all-the-icons-lyellow  :foreground (bytx-lighten yellow 0.3))
    (all-the-icons-dyellow  :foreground (bytx-darken yellow 0.3))
    (all-the-icons-blue     :foreground blue)
    (all-the-icons-blue-alt :foreground teal)
    (all-the-icons-lblue    :foreground (bytx-lighten blue 0.3))
    (all-the-icons-dblue    :foreground dark-blue)
    (all-the-icons-maroon   :foreground magenta)
    (all-the-icons-lmaroon  :foreground (bytx-lighten magenta 0.3))
    (all-the-icons-dmaroon  :foreground (bytx-darken magenta 0.3))
    (all-the-icons-purple   :foreground violet)
    (all-the-icons-lpurple  :foreground (bytx-lighten violet 0.3))
    (all-the-icons-dpurple  :foreground (bytx-darken violet 0.3))
    (all-the-icons-cyan     :foreground cyan)
    (all-the-icons-cyan-alt :foreground cyan)
    (all-the-icons-lcyan    :foreground (bytx-lighten cyan 0.3))
    (all-the-icons-dcyan    :foreground dark-cyan)
    (all-the-icons-pink     :foreground (bytx-lighten red 0.35))
    (all-the-icons-lpink    :foreground (bytx-lighten red 0.55))
    (all-the-icons-dpink    :foreground red)
    (all-the-icons-silver   :foreground (bytx-lighten grey 0.45))
    (all-the-icons-lsilver  :foreground (bytx-lighten grey 0.7))
    (all-the-icons-dsilver  :foreground (bytx-lighten grey 0.1))
    ...
    (avy-lead-face-0
     ...
     (&dark  :background (bytx-lighten highlight 0.3))
     (&light :background (bytx-darken highlight 0.3)))
    (avy-lead-face-1
     (&all   :inherit 'avy-lead-face)
     (&dark  :background (bytx-lighten highlight 0.6))
     (&light :background (bytx-darken highlight 0.6)))
    (avy-lead-face-2
     (&all   :inherit 'avy-lead-face)
     (&dark  :background (bytx-lighten highlight 0.9))
     (&light :background (bytx-darken highlight 0.9)))
    ...
    (cfw:face-header             :foreground (bytx-blend blue bg 0.8) :weight 'bold)
    (cfw:face-sunday             :foreground (bytx-blend red bg 0.8)  :weight 'bold)
    (cfw:face-saturday           :foreground (bytx-blend red bg 0.8)  :weight 'bold)
    ...
    (company-tooltip-search-selection :background (bytx-darken selection 0.25))
    ...
    (diff-context
     (&dark  :foreground (bytx-darken fg 0.12))
     (&light :foreground (bytx-lighten fg 0.12)))
    ...
    ;; dired-subtree
    (dired-subtree-depth-1-face :background (bytx-darken bg-alt 0.02))
    (dired-subtree-depth-2-face :background (bytx-darken bg-alt 0.04))
    (dired-subtree-depth-3-face :background (bytx-darken bg-alt 0.06))
    (dired-subtree-depth-4-face :background (bytx-darken bg-alt 0.08))
    (dired-subtree-depth-5-face :background (bytx-darken bg-alt 0.10))
    (dired-subtree-depth-6-face :background (bytx-darken bg-alt 0.12))

    ;; diredfl
    (diredfl-autofile-name          :foreground base4)
    (diredfl-compressed-file-name   :foreground yellow)
    (diredfl-compressed-file-suffix :foreground (bytx-blend orange bg 0.6))
    (diredfl-date-time              :foreground cyan :weight 'light)
    (diredfl-deletion               :foreground red :background (bytx-blend red bg 0.2) :weight 'bold)
    (diredfl-deletion-file-name     :foreground red :background (bytx-blend red bg 0.2))
    ...
    (diredfl-file-suffix            :foreground (bytx-blend fg bg 0.6))
    (diredfl-flag-mark              :foreground yellow :background (bytx-blend yellow bg 0.2) :weight 'bold)
    (diredfl-flag-mark-line         :background (bytx-blend yellow bg 0.1))
    ...
    ;; bytx-modeline
    (bytx-modeline-eldoc-bar :background green)
    (bytx-modeline-bar-inactive :background nil) ; transparent
    ...
    (erc-header-line :background (bytx-darken bg-alt 0.15) :foreground highlight)
    ...
    (gnus-summary-high-ancient   :foreground (bytx-lighten base5 0.2) :inherit 'italic)
    (gnus-summary-high-read      :foreground (bytx-lighten fg 0.2))
    (gnus-summary-high-ticked    :foreground (bytx-lighten magenta 0.2))
    (gnus-summary-high-unread    :foreground (bytx-lighten green 0.2))
    (gnus-summary-low-ancient    :foreground (bytx-darken base5 0.2) :inherit 'italic)
    (gnus-summary-low-read       :foreground (bytx-darken fg 0.2))
    (gnus-summary-low-ticked     :foreground (bytx-darken magenta 0.2))
    (gnus-summary-low-unread     :foreground (bytx-darken green 0.2))
    ...
    ;; hideshow
    (+bytx-folded-face :inherit 'font-lock-comment-face
                          :weight 'light
                          :background (bytx-darken bg 0.125))
    ...
    (ivy-minibuffer-match-face-1
     :background nil
     :foreground (bytx-lighten grey 0.14)
     :weight 'light)
    ...
    ;; ivy-posframe
    (ivy-posframe :background (bytx-darken bg-alt 0.2))
    ...
    (bytx-neotree-dir-face :foreground highlight)
    (bytx-neotree-file-face :foreground base8)
    (bytx-neotree-hidden-file-face :foreground comments)
    (bytx-neotree-text-file-face :foreground fg)
    (bytx-neotree-data-file-face :foreground violet)
    (bytx-neotree-media-file-face :inherit 'bytx-neotree-hidden-file-face)
    ...
    (lsp-ui-peek-header :foreground fg :background (bytx-lighten bg 0.1) :bold bold)
    (lsp-ui-peek-selection :foreground bg :background blue :bold bold)
    (lsp-ui-peek-list :background (bytx-darken bg 0.1))
    (lsp-ui-peek-peek :background (bytx-darken bg 0.1))
    ...
    (magit-diff-added             :foreground (bytx-darken green 0.2)  :background (bytx-blend green bg 0.1) :extend t)
    (magit-diff-added-highlight   :foreground green                    :background (bytx-blend green bg 0.2) :weight 'bold :extend t)
    (magit-diff-base              :foreground (bytx-darken orange 0.2) :background (bytx-blend orange bg 0.1) :extend t)
    (magit-diff-base-highlight    :foreground orange                   :background (bytx-blend orange bg 0.2) :weight 'bold :extend t)
    (magit-diff-context           :foreground (bytx-darken fg 0.4) :background bg :extend t)
    ...
    (magit-diff-hunk-heading           :foreground bg                    :background (bytx-blend violet bg 0.3) :extend t)
    (magit-diff-hunk-heading-highlight :foreground bg                    :background violet :weight 'bold :extend t)
    (magit-diff-removed                :foreground (bytx-darken red 0.2) :background (bytx-blend red base3 0.1) :extend t)
    (magit-diff-removed-highlight      :foreground red                   :background (bytx-blend red base3 0.2) :weight 'bold :extend t)
    ...
    (objed-hl        :inherit 'region :background (bytx-blend region bg 0.5))
    ...
    (powerline-active1   :inherit 'mode-line :background (bytx-lighten 'bg 0.025))
    (powerline-active2   :inherit 'mode-line :foreground base8 :background (bytx-lighten 'bg 0.08))
    (powerline-inactive0 :inherit 'mode-line-inactive :background base2)
    (powerline-inactive1 :inherit 'mode-line-inactive :background (bytx-lighten 'base2 0.02))
    (powerline-inactive2 :inherit 'mode-line-inactive :background (bytx-lighten 'base2 0.04))
    ...
    ;; smerge-tool
    (smerge-lower :background (bytx-blend green bg 0.2))
    (smerge-upper :background (bytx-blend red base3 0.2))
    (smerge-base  :background (bytx-blend blue bg 0.2))
    ...
    (tldr-introduction     :foreground (bytx-blend blue bg 0.8) :weight 'semi-bold)
    ...
    ;; twittering-mode
    (twitter-divider                    ; custom face in bytx Emacs
     (&light :underline `(:color ,(bytx-lighten vertical-bar 0.2)))
     (&dark  :underline `(:color ,(bytx-darken vertical-bar 0.2))))
    ...
    (font-latex-sectioning-3-face :foreground (bytx-lighten blue 0.3)    :weight 'semi-bold)
    (font-latex-sectioning-4-face :foreground (bytx-lighten magenta 0.3) :weight 'semi-bold)
    (font-latex-sectioning-5-face :foreground (bytx-lighten violet 0.3)  :weight 'semi-bold)
    ...
    (notmuch-search-flagged-face             :foreground (bytx-blend red base4 0.5))
    ...
    (outline-4 :foreground (bytx-lighten blue 0.25)    :weight 'bold :extend t)
    (outline-5 :foreground (bytx-lighten magenta 0.25) :weight 'bold :extend t)
    (outline-6 :foreground (bytx-lighten blue 0.5)     :weight 'bold :extend t)
    (outline-7 :foreground (bytx-lighten magenta 0.5)  :weight 'bold :extend t)
    (outline-8 :foreground (bytx-lighten blue 0.8)     :weight 'bold :extend t)
    ...
    (org-agenda-date-today    :foreground (bytx-lighten violet 0.4)   :weight 'ultra-bold)
    (org-agenda-date-weekend  :foreground (bytx-darken violet 0.4)  :weight 'ultra-bold)
    (org-agenda-structure     :foreground fg :weight 'ultra-bold)
    (org-agenda-clocking      :background (bytx-blend blue bg 0.2))
    (org-upcoming-deadline         :foreground (bytx-blend fg bg 0.8))
    (org-upcoming-distant-deadline :foreground (bytx-blend fg bg 0.5))
    ...
    (org-habit-ready-face          :weight 'bold :background (bytx-blend blue bg-alt 0.5))
    (org-habit-ready-future-face   :weight 'bold :background (bytx-blend blue bg-alt 0.3))
    (org-habit-alert-face          :weight 'bold :background (bytx-blend yellow bg-alt 0.5))
    (org-habit-alert-future-face   :weight 'bold :background (bytx-blend yellow bg-alt 0.3))
    (org-habit-overdue-face        :weight 'bold :background (bytx-blend red bg-alt 0.5))
    (org-habit-overdue-future-face :weight 'bold :background (bytx-blend red bg-alt 0.3))
    ...
    (telephone-line-evil-insert :background (bytx-blend green bg 0.5) :weight 'bold)
    (telephone-line-evil-normal :background (bytx-blend red bg 0.5) :weight 'bold)
    (telephone-line-evil-visual :background (bytx-blend orange bg 0.5) :weight 'bold)
    (telephone-line-evil-replace :background (bytx-color bg-alt) :weight 'bold)
    (telephone-line-evil-motion :background (bytx-blend blue bg 0.5) :weight 'bold)
    (telephone-line-evil-operator :background (bytx-blend violet bg 0.5) :weight 'bold)
    (telephone-line-evil-emacs :background (bytx-blend magenta bg 0.5) :weight 'bold)

    ;; typescript-mode
    (typescript-jsdoc-tag :foreground doc-comments)
    (typescript-jsdoc-type :foreground (bytx-darken doc-comments 0.15))
    (typescript-jsdoc-value :foreground (bytx-lighten doc-comments 0.15))
...
(defvar bytx-themes-base-vars
  '(
    (ansi-color-names-vector
     (vconcat (mapcar #'bytx-color '(bg red green yellow blue magenta cyan fg))))
    (rustic-ansi-faces
     (vconcat (mapcar #'bytx-color '(bg red green yellow blue magenta cyan fg))))

    (fci-rule-color (bytx-color 'base5))

    (jdee-db-spec-breakpoint-face-colors `(cons ,(bytx-color 'base0) ,(bytx-color 'grey)))
    (jdee-db-requested-breakpoint-face-colors `(cons ,(bytx-color 'base0) ,(bytx-color 'green)))
    (jdee-db-active-breakpoint-face-colors `(cons ,(bytx-color 'base0) ,(bytx-color 'highlight)))

    (objed-cursor-color (bytx-color 'red))

    (pdf-view-midnight-colors `(cons ,(bytx-color 'fg) ,(bytx-color 'bg)))

    (vc-annotate-color-map
     `(list (cons 20  ,(bytx-color 'green))
            (cons 40  ,(bytx-blend 'yellow 'green (/ 1.0 3)))
            (cons 60  ,(bytx-blend 'yellow 'green (/ 2.0 3)))
            (cons 80  ,(bytx-color 'yellow))
            (cons 100 ,(bytx-blend 'orange 'yellow (/ 1.0 3)))
            (cons 120 ,(bytx-blend 'orange 'yellow (/ 2.0 3)))
            (cons 140 ,(bytx-color 'orange))
            (cons 160 ,(bytx-blend 'magenta 'orange (/ 1.0 3)))
            (cons 180 ,(bytx-blend 'magenta 'orange (/ 2.0 3)))
            (cons 200 ,(bytx-color 'magenta))
            (cons 220 ,(bytx-blend 'red 'magenta (/ 1.0 3)))
            (cons 240 ,(bytx-blend 'red 'magenta (/ 2.0 3)))
            (cons 260 ,(bytx-color 'red))
            (cons 280 ,(bytx-blend 'grey 'red (/ 1.0 4)))
            (cons 300 ,(bytx-blend 'grey 'red (/ 2.0 4)))
            (cons 320 ,(bytx-blend 'grey 'red (/ 3.0 4)))
            (cons 340 ,(bytx-color 'base5))
            (cons 360 ,(bytx-color 'base5))))
    (vc-annotate-very-old-color nil)
    (vc-annotate-background (bytx-color 'bg)))
  "TODO")

(defvar bytx-theme-status "init")

(defun bytx-theme-load-light ()
  ;; 禁用当前主题
  (mapc #'disable-theme custom-enabled-themes)
  (load-theme 'bytx-light t)
  (setq bytx-theme-status "light"))

(defun bytx-theme-load-dark ()
  (mapc #'disable-theme custom-enabled-themes)
  (load-theme 'bytx-dark t)
  (setq bytx-theme-status "dark"))

(defun bytx-theme-toggle ()
  (interactive)
  (let ((bg-mode (frame-parameter nil 'background-mode)))
    (if (eq bg-mode 'dark)
        (bytx-theme-load-light)
      (bytx-theme-load-dark))))

(defun bytx-theme-is-day ()
  (let ((current-hour (string-to-number (format-time-string "%H"))))
    (and (> current-hour 7)
         (< current-hour 18))))

(defun bytx-theme-load ()
  "根据当前时间与 bytx-theme-status 的情况切换亮与暗主题"
  (if (bytx-theme-is-day)
      (when (or (string-equal bytx-theme-status "init")
                (string-equal bytx-theme-status "dark"))
        (bytx-theme-load-light))
    (when (or (string-equal bytx-theme-status "init")
              (string-equal bytx-theme-status "light"))
      (bytx-theme-load-dark))))

(defun bytx-theme-load-with-sunrise ()
  "自动每小时检测一次主题并变换"
  (run-with-timer 0 (* 60 60) 'bytx-theme-load))
